...
'Record STUDENT Item Class @2-E07644F4
...
'Record STUDENT Data Provider Class Variables @2-AF5E1CAF
...
'Record STUDENT Data Provider Class Constructor @2-35FD94D6
    ...
    Public Sub New()
        Select_=new TableCommand("SELECT MIDDLE_NAME AS MIDDLE_NAME, STUDENT_ID, SURNAME AS SURNAME, " & vbCrLf & _
          ...
          "SEX_BIRTH " & vbCrLf & _
          "FROM STUDENT {SQL_Where} {SQL_OrderBy}", New String(){"STUDENT_ID328"},Settings.connDECVPRODSQL2005DataAccessObject )
        Select_.OrderBy=""
    End Sub
...
'Record STUDENT Data Provider Class CheckUnique Method @2-01AEBC10
    ...
    Public Function CheckUnique(ByVal ControlName As String, ByVal item As STUDENTItem) As Boolean
        Dim Check As TableCommand = New TableCommand("SELECT COUNT(*)" & vbCrLf & _
          "FROM STUDENT",New String(){"STUDENT_ID328"},Settings.connDECVPRODSQL2005DataAccessObject)
        ...
        CType(Check,TableCommand).AddParameter("STUDENT_ID328",UrlSTUDENT_ID, "","STUDENT_ID",Condition.Equal,False)
...
'Record STUDENT Build insert @2-8A309591
...
'Record STUDENT Data Provider Class Update Method @2-5A38743A

    Public Function UpdateItem(ByVal item As STUDENTItem) As Integer
        Me.item = item
        Update=new TableCommand("UPDATE STUDENT SET {Values}", New String(){"STUDENT_ID328"},Settings.connDECVPRODSQL2005DataAccessObject , true)
'End Record STUDENT Data Provider Class Update Method

'Record STUDENT BeforeBuildUpdate @2-48F9F4D4
		Dim valuesList As String = ""
		Dim allEmptyFlag As Boolean= True
		
        Update.Parameters.Clear()
        CType(Update,TableCommand).AddParameter("STUDENT_ID328",UrlSTUDENT_ID, "","STUDENT_ID",Condition.Equal,False)
        If Not item.SURNAME.IsEmpty Then
        allEmptyFlag = item.SURNAME.IsEmpty And allEmptyFlag
...
'Record STUDENT BeforeBuildSelect @2-F96C35FC
        Select_.Parameters.Clear()
        CType(Select_,TableCommand).AddParameter("STUDENT_ID328",UrlSTUDENT_ID, "","STUDENT_ID",Condition.Equal,False)
...
'Record STUDENT AfterExecuteSelect @2-DE583582
                If Not IsNothing(E) Then Throw E
        End Try
        End If
