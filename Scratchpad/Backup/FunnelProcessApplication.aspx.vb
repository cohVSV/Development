...
' TODO: Rework the initial display logic to bail out if the lead isn't in the FUNNEL_LEAD table?
...
Partial Public Class FunnelProcessApplicationPage
   Inherits CCPage
   ...
   ' Test leads: e94fef64-7dd9-4052-93e7-c43117feebac (Smith surname, to test matching against lots of preliminary surname matches)
   '             fa40e332-c7f1-47e2-8324-1ac2a80e1b4a (Kentucky Bluegrass)
   '             faac7ad7-2ae8-4ea0-bfb1-f57bfc3ba81f (Ford Prefect - Direct, Medical)
   '             d1d9170c-8f76-4d70-8974-eaced85bfd9d (Duke Brigerton - has preferred name)
   '             ae58eb72-78ed-4cf3-8451-0ebae40befb6 (Gordon Summers - Direct, Medical, comprehensive details for optional fields)
   '             cce352c2-40b2-4926-82f3-baa6d3bb2df7 (Delete Me - a deleted lead)
   '             4725050d-6bd5-47ed-946f-d612863f1a34 (William Batts - Direct, Medical (Social/Emotional))
   '             6d1c7ddb-e764-477a-8557-d252da6a77bc (Chelsea Jones - Replica of existing VSV student)
   '             187272ac-d76b-4c19-9d92-661f00578ba9 (Terry Towelling - School-based)
   '             296d7a64-a613-42d1-b951-8be732e4d605 (Anthony Lazuto - School-based)
   ...
   ' TODO: Remove these category codes if they're not needed, now that we're using views
   Const YearLevelCategory = "YearLevel"
   Const GenderCategory = "Gender"
   Const RelationshipTypeCategory = "RelationshipType"
   Const ResidentialStatusCategory = "ResidentialStatus"
   Const CountryAlpha2CodeCategory = "CountryAlpha2Code"
   Const HouseholdStatusCategory = "HouseholdStatus"
   Const IndigenousStatusCategory = "IndigenousStatus"
   Const ParentSchoolEducationCategory = "ParentSchoolEducation"
   Const ParentNonSchoolEducationCategory = "ParentQualification"
   Const ParentOccupationGroupCategory = "ParentOccupationGroup"
   Const SupportProgramServiceCategory = "SupportProgramService"
   Const PractitionerTypeCategory = "PractitionerType"

   'Page constants
   Const PageTitle = "Import Applicant from Funnel to VSV Student Database"
   ...
   Private Shared Function GetAgeAtDate(dateOfBirth As Date, Optional atDate As Date? = Nothing) As Integer
      Dim atDateLocal = If(atDate IsNot Nothing, atDate.Value, Date.Now)
   ...
   Private Shared Function FormatPhoneNumber(phoneNumber As String) As String
      ' Always remove the Aus country prefix, if present
   ...
   Private Sub Page_InitComplete(sender As Object, e As EventArgs) Handles Me.InitComplete
      ' CCPage.OnInit will be invoked prior to this
      ' Authorise for data entry staff, enrolments staff, and admin level users (SYSTEM group)
      If Not (DBUtility.AuthorizeUser(New String() {"4", "6", "9"})) Then
         Response.Redirect(Settings.ServerURL)
      End If
   End Sub
   ...
   ' TODO:
   ' - Bail out gracefully if the lead on Funnel hasn't been.. submitted or completed?
   ...
   Private Async Function InitialiseFunnelStudentImportPage() As Task
      Try
         ...
         Using connection = New SqlConnection(Settings.connDECVPRODSQL2005Connection.Connection)
            ...
            DestinationStudentID = If(importStatus?.ImportedStudentID IsNot Nothing, importStatus.ImportedStudentID.Value, NoStudentID)
            ...
            Else
               ...
               ' Check that the student hasn't already been imported into the student database
               ...
               If (importStatus?.ImportedStudentID Is Nothing) Then
                  ...
                  Else
                     ...
                     Else
                        ...
                        ' TODO: disable ability to go back to the existing student selection page
      ...
      Catch ex As Exception
         ShowDivMessage(divFunnelStudentImportError, "An error has occurred:" & vbNewLine & vbNewLine & ex.Message & vbNewLine & vbNewLine & ex.StackTrace)
      End Try
   End Function


   ' --------------------------------------------------------------------------------------------------------------------------------------
   ' Begin common rendering methods
   ' --------------------------------------------------------------------------------------------------------------------------------------
   ...
   Private Shared Sub ShowDivMessage(divElement As HtmlGenericControl, message As String)
   ...
   Private Sub InitialiseUnknownLeadDisplay()
      ...
      divApplicantAlert.InnerHtml = "Unknown Funnel applicant lead ID. The lead either does not exist or has been deleted without being imported into the student database."
   ...
   Private Sub InitialiseImportStatusControl(importStatus As FunnelLeadSDBImportStatus)
      With importStatus
         If ((?.ApplicationSubmittedImportedDate Is Nothing) AndAlso (?.ApplicationCompletedImportedDate Is Nothing)) Then
            ltApplicantImportStatus.Text = "This applicant has not yet been imported into the VSV student database"
         Else
            Dim sdbLink = Common.GenerateStudentSummaryPageLink(.ImportedStudentID.ToString(), .FunnelTargetEnrolmentYear.ToString())
            Dim sdbLinkHtml = $"<a href='{sdbLink}' target='_blank'>{ .ImportedStudentID}</a>"
            Dim newOrExistingStudent = If(.ImportedExistingStudent, "(an existing student)", "(a new student)")
            Dim statusText As String = $"Imported into the database as student ID {sdbLinkHtml} {newOrExistingStudent} for the { .FunnelTargetEnrolmentYear} enrolment year:<ul>"

            If (.ApplicationSubmittedImportedDate IsNot Nothing) Then
               statusText &= GenerateImportStatusText("Application Submitted", .ApplicationSubmittedImportedDate.Value, .ApplicationSubmittedImportedBy)
            End If
            If (.ApplicationCompletedImportedDate IsNot Nothing) Then
               statusText &= GenerateImportStatusText("Application Completed", .ApplicationCompletedImportedDate.Value, .ApplicationCompletedImportedBy)
            End If
   ...
   Private Function GenerateImportStatusText(funnelImportStage As String, importedDate As Date, importedBy As String) As String
      Return $"<li><strong>{Common.GetDateTimeDisplay(importedDate)}</strong>: Imported by {importedBy} at the {funnelImportStage} stage in Funnel</li>"
   End Function
   ...
   Protected Sub btnStudentImportCancel_Click(sender As Object, e As EventArgs) Handles btnStudentImportCancel.Click
      'RegisterAsyncTask(New PageAsyncTask(AddressOf InitialisePageAsync))
      'Dim var = Request.Form("rbgnExistingStudentID")
   End Sub


   'Protected Sub cvrbgnExistingStudentID_ServerValidate(source As Object, args As ServerValidateEventArgs) Handles cvrbgnExistingStudentID.ServerValidate
   '   args.IsValid = (Request.Form("rbgnExistingStudentID") IsNot Nothing)
   'End Sub
   ...
   Protected Sub btnPreviewImport_Click(sender As Object, e As EventArgs) Handles btnPreviewImport.Click
   ...
   Private Async Function InitialisePreviewUpdateViewAsync(importStatus As FunnelLeadSDBImportStatus) As Task
      ...
      Catch ex As Exception
         ' TODO: This message may (also?) need to be shown at the bottom of the preview update view
         ShowDivMessage(divFunnelStudentImportError, "An error has occurred:" & vbNewLine & vbNewLine & ex.Message & vbNewLine & vbNewLine & ex.StackTrace)
      End Try
   End Function
      ...
      With parentCarer
         ...
         Else
            If (existingParentCarer IsNot Nothing) Then
               ...
               If (.StudentConnections.Count > 0) Then
                  Dim student = .StudentConnections(0)
                  ...
                  If (.StudentConnections.Count > 1) Then
                     builder.Append($" and { .StudentConnections.Count - 1} other student(s).")
                  Else
                     builder.Append(".")
                  End If
               End If
   ...
   Private Sub RenderParentCensusDetails(renderer As StudentImportPreviewRenderer, funnelParentCensusDetails As SDBParentCensusData,
      ...
      With funnelParentCensusDetails
         renderer.RenderFieldRow(NameOf(.CountryOfBirth), parentTypeLabelPrefix & "country of birth", .CountryOfBirthDisplayName, existingParentCensusDetails?.CountryOfBirthDisplayName)
         renderer.RenderFieldRow(NameOf(.Language), parentTypeLabelPrefix & "spoken language at home other than English", .LanguageDisplayName, existingParentCensusDetails?.LanguageDisplayName)
         renderer.RenderFieldRow(NameOf(.SchoolEducation), parentTypeLabelPrefix & "highest year of primary or secondary education completed", .SchoolEducationDisplayName, existingParentCensusDetails?.SchoolEducationDisplayName)
         renderer.RenderFieldRow(NameOf(.NonSchoolEducation), parentTypeLabelPrefix & "highest qualification completed", .NonSchoolEducationDisplayName, existingParentCensusDetails?.NonSchoolEducationDisplayName)
         renderer.RenderFieldRow(NameOf(.OccupationGroup), parentTypeLabelPrefix & "occupation group", .OccupationGroupDisplayName, existingParentCensusDetails?.OccupationGroupDisplayName)
      End With
   End Sub
   ...
   Class StudentImportPreviewRenderer
      ...
      Sub RenderTableCustomCellHtml(html As String)
         With htmlBuilder
            ...
            .Append($"<td colspan='{GetColumnSpan()}' style = 'padding-bottom: 15px;' >")
   ...
   Private Function GetApplicantKeyDetailsQuery(uuid As Guid) As String
      ' Also: check where the school ID is stored, for school-based students
   ...
   '
   ' These classes contain the SDB codes after being translated from Funnel enums
   ...
   Class FunnelSDBMappings
      ...
      ' Census mappings
      Friend CountryOfBirth As Short? ' TODO: Get updated list from DET and provide to Digistorm
      ...
      Friend HouseholdStatus As Decimal? ' Lookup of living_arrangement; I think the new values for OoHC might be wrong
   ...
   Private Function GetApplicantEnrolmentMappings(applicant As Lead, connection As SqlConnection) As FunnelSDBMappings
      ...
      With mappings
         ...
         .Gender = GetFunnelEnumSDBValue(Of String)(AccountEnumsOwner, GendersEnumName, applicant.child.gender.uuid.ToString(), connection)
         ...
         If (IsSchoolBasedEnrolmentApplication(applicant.custom_properties.student_enrolment_type)) Then
            ...
            ' TODO: confirm once and for all where to retrieve the home school ID from the Funnel object, to do the lookup properly here
            GetFunnelEnumDisplayNameSDBValue(LeadEnumsOwner, NameOfHomeSchoolEnumName, applicant.custom_properties.name_of_school, .HomeSchoolDisplayName,
                                             .HomeSchoolID, connection)
         ...
         Else
            ...
            ' TODO: Determine where to pick up the correct home school ID and display name for shared enrolments. Attending school will still be VSV.
         End If
   ...
   Class FunnelLeadSDBImportStatus
      ...
      Friend FunnelSDBEnrolmentCategoryCode As String
      Friend FunnelSDBEnrolmentSubcategoryCode As String
      Friend FunnelIsSharedEnrolment As Boolean
      Friend FunnelSDBYearLevel As Short?
      Friend FunnelYearLevelDisplayName As String
      Friend FunnelCreatedDate As Date
      Friend FunnelUpdatedDate As Date

      Friend ImportedStudentID As Decimal?
      Friend ImportedExistingStudent As Boolean
      Friend ImportedEnrolmentCategoryCode As String
      Friend ImportedEnrolmentSubcategoryCode As String
      Friend ApplicationSubmittedImportedBy As String
      Friend ApplicationSubmittedImportedDate As Date?
      Friend ApplicationCompletedImportedBy As String
      Friend ApplicationCompletedImportedDate As Date?
   End Class
   ...
   Class SDBStudent
      ...
      Friend HomeSchoolID As Decimal? ' TODO: unknown, might be Lead.CP.name_of_school, and might need to be mapped
      Friend AttendingSchoolID As Decimal? ' TODO: unknown, might be Lead.CP.name_of_school, and might need to be mapped
      ...
      ' TODO:
      ' Matt had mentioned the need for the SAC address to Tim during a form review meeting, but I'm not sure that it was being added
   ...
   Class SDBStudentCensusData
      ...
      Friend CountryOfBirth As Short? ' This is the code that we provided to Digistorm
      Friend FirstHomeLanguage As Short?
   ...
   ' --------------------------------------------------------------------------------------------------------------------------------------
   ' Begin Funnel lead import status and data matching methods
   ' --------------------------------------------------------------------------------------------------------------------------------------
   ...
   Private Function GetFunnelLeadImportStatus(uuid As Guid, connection As SqlConnection) As FunnelLeadSDBImportStatus
      Using command = connection.CreateCommand()
         ...
         command.CommandText = "select
                                    ...
                                    vfl.ImportedStudentID,
                                    st.CATEGORY_CODE as ImportedEnrolmentCategoryCode,
                                    st.SUBCATEGORY_CODE as ImportedEnrolmentSubcategoryCode,
                                    vfl.ApplicationSubmittedImportedBy,
                                    vfl.ApplicationSubmittedImportedDate,
                                    vfl.ApplicationCompletedImportedBy,
                                    vfl.ApplicationCompletedImportedDate,
                                    cast(iif((vfl.ImportedStudentID is not null)
                                             and exists
                                                 (
                                                    select
                                                       *
                                                     ...
                                                     where
                                                       (se.STUDENT_ID = vfl.ImportedStudentID)
                                                       and (se.ENROLMENT_YEAR < vfl.FunnelTargetEnrolmentYear)
                                                 ),
                                            'true',
                                            'false') as bit) as ImportedExistingStudent
                                  from
                                    dbo.vwFunnelLead as vfl
                                    left join dbo.STUDENT as st
                                       on (st.STUDENT_ID = vfl.ImportedStudentID)
                                  where
                                    (vfl.FunnelLeadUUID = @leadUUID);"

         Using reader = command.ExecuteReader()
            If reader.Read() Then
               Dim importStatus As New FunnelLeadSDBImportStatus() With
            {
               ...
               .FunnelSDBEnrolmentCategoryCode = reader.Value(Of String)("FunnelSDBEnrolmentCategoryCode"),
               .FunnelSDBEnrolmentSubcategoryCode = reader.Value(Of String)("FunnelSDBEnrolmentSubcategoryCode"),
               .FunnelIsSharedEnrolment = IsSharedEnrolmentApplication(reader.Value(Of String)("FunnelIsSharedEnrolmentID")),
               .FunnelSDBYearLevel = reader.Value(Of Short?)("FunnelSDBYearLevel"),
               .FunnelYearLevelDisplayName = reader.Value(Of String)("FunnelYearLevel"),
               .FunnelCreatedDate = reader.Value(Of Date)("FunnelCreatedDate"),
               .FunnelUpdatedDate = reader.Value(Of Date)("FunnelUpdatedDate"),
               .ImportedExistingStudent = reader.Value(Of Boolean)("ImportedExistingStudent"),
               .ImportedStudentID = reader.Value(Of Decimal?)("ImportedStudentID"),
               .ImportedEnrolmentCategoryCode = reader.Value(Of String)("ImportedEnrolmentCategoryCode"),
               .ImportedEnrolmentSubcategoryCode = reader.Value(Of String)("ImportedEnrolmentSubcategoryCode"),
               .ApplicationSubmittedImportedBy = reader.Value(Of String)("ApplicationSubmittedImportedBy"),
               .ApplicationSubmittedImportedDate = reader.Value(Of Date?)("ApplicationSubmittedImportedDate"),
               .ApplicationCompletedImportedBy = reader.Value(Of String)("ApplicationCompletedImportedBy"),
               .ApplicationCompletedImportedDate = reader.Value(Of Date?)("ApplicationCompletedImportedDate")
            }
               Return importStatus
            End If
         End Using
                                 ...
                                 StudentKeyDetails as
                                    (
                                       select
                                        ...
                                        where
                                          (se.ENROLMENT_YEAR >= @minimumEnrolmentYear)
                                          and
                                          (
   ...
   Private Function GetMatchingExistingStudents(applicantKeyDetails As Lead, preliminaryMatches As List(Of SDBStudentMatch)) As List(Of SDBStudentMatch)
      ...
      For Each student In preliminaryMatches
         ' Normalise the student key fields
                                    ...
                                    LinkedSchool as
                                       (
                                          select top 1
                                             ...
                                             dbo.ProperCase(sch.SCHOOL_NAME) as LinkedSchoolDisplayName
                                           from
                                             dbo.SCHOOL as sch
                                           where
                                    ...
                                    CarerStudentConnections as
                                       (
                                          select
                                             ...
                                             row_number() over (partition by vscc.STUDENT_ID, vscc.CARER_TYPE order by vscc.ENROLMENT_YEAR desc) as StudentEnrolmentYearRowNumber,
                                           ...
                                           where
                                             (vscc.ENROLMENT_YEAR >= @studentConnectionMinimumEnrolmentYear)
