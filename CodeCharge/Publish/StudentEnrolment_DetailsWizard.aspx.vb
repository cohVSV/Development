...
'End Page_Load Event
            ...
            With Me
                ...
                If .radioSex.SelectedValue = "M" Then
...
'Page class tail @1-DD082417
        ...
        Protected Sub Wizard_AddStudent_FinishButtonClick(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.WizardNavigationEventArgs) Handles Wizard_AddStudent.FinishButtonClick
            ...
            If (Me.hidden_StudentEnrolled.Value = "0") Then
                ...
                Try
                    ...
                    cmd.Parameters.Clear()
                    strSQL = "INSERT INTO [STUDENT](SURNAME,FIRST_NAME,MIDDLE_NAME,BIRTH_DATE,SEX,CATEGORY_CODE,SUBCATEGORY_CODE,ATTENDING_SCHOOL_ID,HOME_SCHOOL_ID,POSTAL_ADDRESS_ID,CURR_RESID_ADDRESS_ID,ORIG_RESID_ADDRESS_ID,FULL_TIME,BULLETIN_NUMBER,OS_FULL_FEE_PAYING,ENROLMENT_COMMENTS,VCE_CANDIDATE_NUMBER,ADDRESS_REVIEW_FLAG,LAST_ALTERED_BY,LAST_ALTERED_DATE,PARENT1,PARENT2,REGION,PREFERRED_NAME,VSN,ENROLLEDBEFORE,VERIFY,DATE_STUDENTFOLDER_CREATED, STUDENT_EMAIL, STUDENT_MOBILE)"
                    strSQL += " VALUES (@SURNAME,@FIRST_NAME,@MIDDLE_NAME,@BIRTH_DATE,@SEX,@CATEGORY_CODE,@SUBCATEGORY_CODE,@ATTENDING_SCHOOL_ID,@HOME_SCHOOL_ID,@POSTAL_ADDRESS_ID,@CURR_RESID_ADDRESS_ID,@ORIG_RESID_ADDRESS_ID,@FULL_TIME,@BULLETIN_NUMBER,@OS_FULL_FEE_PAYING,@ENROLMENT_COMMENTS,@VCE_CANDIDATE_NUMBER,@ADDRESS_REVIEW_FLAG,@LAST_ALTERED_BY,GetDate(),@PARENT1,@PARENT2,@REGION,@PREFERRED_NAME,@VSN,@ENROLLEDBEFORE,@VERIFY,@DATE_STUDENTFOLDER_CREATED,@STUDENT_EMAIL,@STUDENT_MOBILE)"

                    cmd.CommandText = strSQL
                    cmd.Parameters.AddWithValue("SURNAME", IIf(txtSurname.Text <> "", prepStringForSQLNU(txtSurname.Text), DBNull.Value)) ' LN: 28/11/2013 prepStringForSQLNU
                    cmd.Parameters.AddWithValue("FIRST_NAME", IIf(txtFirstName.Text <> "", prepStringForSQLNU(txtFirstName.Text), DBNull.Value)) ' LN: 28/11/2013 prepStringForSQLNU
                    cmd.Parameters.AddWithValue("MIDDLE_NAME", IIf(txtMiddleName.Text <> "", prepStringForSQLNU(txtMiddleName.Text), DBNull.Value)) ' LN: 28/11/2013 prepStringForSQLNU
                    ...
                    cmd.Parameters.AddWithValue("SEX", Me.radioSex.SelectedValue.ToUpper())
                    ...
                    cmd.Parameters.AddWithValue("ENROLMENT_COMMENTS", IIf(Me.txtareacomments.Text <> "", prepStringForSQL(Me.txtareacomments.Text), DBNull.Value))
                    ...
                    cmd.Parameters.AddWithValue("PREFERRED_NAME", DBNull.Value)
                    ...
                    cmd.Parameters.AddWithValue("STUDENT_EMAIL", IIf(Me.txtStudentEmail.Text <> "", prepStringForSQL(Me.txtStudentEmail.Text), DBNull.Value))
                    ...
                    If Me.txtareacomments.Text <> "" Then
                        ...
                        strSQL += "select (select (max(COMMENT_ID+1)) from STUDENT_COMMENT) ," & tmpNewStudentID.ToString & ",'" & prepStringForSQL(Me.txtareacomments.Text) & "' "
                        strSQL += ",'" & DBUtility.UserLogin.ToUpper & "', getdate(),'A','ENROLMENT'"
                        cmd.CommandText = strSQL
                        cmd.ExecuteNonQuery()
                    End If
        ...
        ' LN: 28/8/2013
        Public Function prepStringForSQLNU(ByVal sValue As String) As String
            Dim sAns As String = ""
            If sValue <> "" Then
                sAns = sValue.Replace("'", "''")
            End If
