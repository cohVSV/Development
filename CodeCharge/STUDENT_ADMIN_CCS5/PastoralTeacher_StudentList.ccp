<Page id="1" templateExtension="html" relativePath="." fullRelativePath="." secured="True" urlType="Relative" isIncluded="False" SSLAccess="False" isService="False" cachingEnabled="False" validateRequest="True" cachingDuration="1 minutes" wizardTheme="Blueprint" wizardThemeVersion="3.0" needGeneration="0">
 ...
 (
    select
       st.*,
       iif((smd.PSD_FUNDING_ASSESSED = 1)
                   or (smd.PSD_FUNDING_ELIGIBLE = 1)
                   or (smd.PSD_FUNDING_LEVEL is not null)
                   or (len(smd.PSD_FUNDING_CATEGORY) &gt; 2)
                   or (smd.RECEIVED_SUPPORT_PROGRAMS_SERVICES like '%,PSD%'),
       'PSD Funded; ',
       '') as PSDFunded,
       iif((smd.NCCD_FUNDING_APPROVED = 1)
                   or (smd.NCCD_FUNDING_CATEGORY is not null)
                   or (smd.NCCD_FUNDING_LEVEL is not null), 'NCCD Funded; ', '') as NCCDFunded,
       iif(scd.HOUSEHOLD_STATUS in (3, 6, 7), concat(vhs.HouseholdStatusDisplayText, '; '), '') as HouseholdStatus,
       iif(scd.KOORITORRESFLAG in ('K', 'B', 'T'), 'Indigenous; ', '') as ATSIStatus,
       iif(scd.YOUTH_JUSTICE_INVOLVEMENT = 1, 'Youth Justice; ', '') as YouthJustice,
     ...
     where
       ...
       and (st.ENROLMENT_YEAR = year(getdate()))
			...
			<SPParameters/>
			<SQLParameters>
				<SQLParameter id="296" dataType="Text" defaultValue="&quot;&quot;" designDefaultValue="MKING" parameterSource="UserId" parameterType="Session" variable="UserId"/>
</SQLParameters>
			<JoinTables>
			</JoinTables>
			<JoinLinks/>
			...
			<UConditions>
				...
				<TableParameter id="132" conditionType="Expression" useIsNull="False" field="SUBJECT_ID" dataType="Integer" searchConditionType="Equal" parameterType="URL" logicOperator="And" expression="SUBJECT_ID not in (select vis.IntroductorySubjectID from dbo.vwIntroductorySubject as vis where (vis.IntroductorySubjectStatus = 1)) " parameterSource="SUBJECT_ID"/>
			</UConditions>
			<UFormElements>
 ...
 (
    select
       ...
       iif(scd.KOORITORRESFLAG in ('K', 'B', 'T'), 'Indigenous; ', '') as ATSIStatus,
       iif(scd.YOUTH_JUSTICE_INVOLVEMENT = 1, 'Youth Justice; ', '') as YouthJustice,
       iif(eas.AcademicLag = 1, 'Assessed as two years or more below expected level; ', '') as AcademicLag
     from
        ...
        select
         ...
         where
           ...
           and (ss.STAFF_ID = '{UserId}')
 ...
 ) as subq" pageSizeLimit="100" customUpdateType="Table" customUpdate="student_subject" customDeleteType="Procedure" customDelete="spu_SSTeacher_StudentsNoCLP;1" PathID="viewMaintainSearchRequest2" activeCollection="UConditions" activeTableType="customUpdate">
			<Components>
			...
			<SPParameters/>
			<SQLParameters>
				<SQLParameter id="295" dataType="Text" defaultValue="&quot;&quot;" designDefaultValue="AONEILL" parameterSource="UserId" parameterType="Session" variable="UserId"/>
</SQLParameters>
			<JoinTables/>
			<JoinLinks/>
			<Fields/>
