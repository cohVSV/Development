...
'Forms Objects @1-7753BAF9
    ...
    Protected Grid_Modified_CommentData As Grid_Modified_CommentDataProvider
    Protected Grid_Modified_CommentOperations As FormSupportedOperations
    Protected Grid_Modified_CommentCurrentRowNumber As Integer
...
'Page_Load Event BeforeIsPostBack @1-77E95698
...
'Record Form viewStudentSummary_Detail Show method @4-7F88CDD0
...
'Record Form viewStudentSummary_Detail BeforeShow tail @4-02C4570C
...
'Grid Grid_Modified_Comment Bind @428-770A4C81

    Protected Sub Grid_Modified_CommentBind()
        If Not Grid_Modified_CommentOperations.AllowRead Then Return
        Dim PagesCount As Integer
        Dim FooterIndex As Integer
        If Not(IsPostBack) Then
            DBUtility.InitializeGridParameters(ViewState,"Grid_Modified_Comment",GetType(Grid_Modified_CommentDataProvider.SortFields), 10, 100)
        End If
'End Grid Grid_Modified_Comment Bind

'Grid Form Grid_Modified_Comment BeforeShow tail @428-620AA70C
        Grid_Modified_CommentParameters()
        Grid_Modified_CommentData.SortField = CType(ViewState("Grid_Modified_CommentSortField"),Grid_Modified_CommentDataProvider.SortFields)
        Grid_Modified_CommentData.SortDir = CType(ViewState("Grid_Modified_CommentSortDir"),SortDirections)
        Grid_Modified_CommentData.PageNumber = CInt(ViewState("Grid_Modified_CommentPageNumber"))
        Grid_Modified_CommentData.RecordsPerPage = CInt(ViewState("Grid_Modified_CommentPageSize"))
        Grid_Modified_CommentRepeater.DataSource = Grid_Modified_CommentData.GetResultSet(PagesCount, Grid_Modified_CommentOperations)
        ViewState("Grid_Modified_CommentPagesCount") = PagesCount
        Dim item As Grid_Modified_CommentItem = New Grid_Modified_CommentItem()
        Grid_Modified_CommentRepeater.DataBind
        FooterIndex = Grid_Modified_CommentRepeater.Controls.Count - 1
        If PagesCount = 0 Then
            Grid_Modified_CommentRepeater.Controls(FooterIndex).FindControl("NoRecords").Visible = True
        End If

'End Grid Form Grid_Modified_Comment BeforeShow tail

'Grid Grid_Modified_Comment Event BeforeShow. Action Custom Code @447-73254650
    ' -------------------------
    ' ERA: 14-Feb-2013|EA| new Modified Subject comments - if nothing then don't show it
	If PagesCount = 0 Then 
    	Grid_Modified_CommentRepeater.Visible = False
	End if
    ' -------------------------
'End Grid Grid_Modified_Comment Event BeforeShow. Action Custom Code

'Grid Grid_Modified_Comment Bind tail @428-E31F8E2A
    End Sub
'End Grid Grid_Modified_Comment Bind tail

'Grid Grid_Modified_Comment Table Parameters @428-18347BE1

    Protected Sub Grid_Modified_CommentParameters()
        Try
            Grid_Modified_CommentData.UrlSTUDENT_ID = FloatParameter.GetParam("STUDENT_ID", ParameterSourceType.URL, "", Nothing, false)
            Grid_Modified_CommentData.UrlENROLMENT_YEAR = IntegerParameter.GetParam("ENROLMENT_YEAR", ParameterSourceType.URL, "", year(now()), false)

        Catch
            Dim ParentControls As ControlCollection=Grid_Modified_CommentRepeater.Parent.Controls
            Dim RepeaterIndex As Integer=ParentControls.IndexOf(Grid_Modified_CommentRepeater)
            ParentControls.RemoveAt(RepeaterIndex)
            Dim ErrorMessage as Literal=New Literal()
            ErrorMessage.Text="Error in Grid Grid_Modified_Comment: Invalid parameter"
            ParentControls.AddAt(RepeaterIndex,ErrorMessage)
        End Try
    End Sub
'End Grid Grid_Modified_Comment Table Parameters

'Grid Grid_Modified_Comment ItemDataBound event @428-E3BF6E7A

    Protected Sub Grid_Modified_CommentItemDataBound(Sender as Object, e as RepeaterItemEventArgs)
        Dim DataItem as Grid_Modified_CommentItem = CType(e.Item.DataItem,Grid_Modified_CommentItem)
        Dim Item as Grid_Modified_CommentItem = DataItem
        Dim FormDataSource As Grid_Modified_CommentItem() = CType(Grid_Modified_CommentRepeater.DataSource, Grid_Modified_CommentItem())
        Dim Grid_Modified_CommentDataRows As Integer = FormDataSource.Length
        Dim Grid_Modified_CommentIsForceIteration As Boolean = False
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            Grid_Modified_CommentCurrentRowNumber += 1
        CType(Page,CCPage).ControlAttributes.Add(Grid_Modified_CommentRepeater,new CCSControlAttribute("rowNumber", FieldType._Integer, Grid_Modified_CommentCurrentRowNumber), AttributeOption.Multiple)
        End If
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            Dim Grid_Modified_CommentCOMMENT As System.Web.UI.WebControls.Literal = DirectCast(e.Item.FindControl("Grid_Modified_CommentCOMMENT"),System.Web.UI.WebControls.Literal)
            Dim Grid_Modified_CommentLAST_ALTERED_BY As System.Web.UI.WebControls.Literal = DirectCast(e.Item.FindControl("Grid_Modified_CommentLAST_ALTERED_BY"),System.Web.UI.WebControls.Literal)
            Dim Grid_Modified_CommentLAST_ALTERED_DATE As System.Web.UI.WebControls.Literal = DirectCast(e.Item.FindControl("Grid_Modified_CommentLAST_ALTERED_DATE"),System.Web.UI.WebControls.Literal)
        End If
'End Grid Grid_Modified_Comment ItemDataBound event

'Grid Grid_Modified_Comment ItemDataBound event tail @428-7CF35E9B
        If Grid_Modified_CommentIsForceIteration Then
            Dim ri As RepeaterItem = Nothing
            ri= New RepeaterItem(Grid_Modified_CommentCurrentRowNumber, ListItemType.Item)
            Grid_Modified_CommentRepeater.ItemTemplate.InstantiateIn(ri)
            ri.DataItem = DataItem
            ri.DataBind()
            e.Item.FindControl("IterationContainer").Controls.Add(ri)
            Grid_Modified_CommentItemDataBound(Sender, New RepeaterItemEventArgs(ri))
            ri.DataItem = Nothing
        End If
    End Sub
'End Grid Grid_Modified_Comment ItemDataBound event tail

'Grid Grid_Modified_Comment ItemCommand event @428-EFF1910A

    Protected Sub Grid_Modified_CommentItemCommand(Sender as Object, e as RepeaterCommandEventArgs)
        Dim FooterIndex as Integer= Grid_Modified_CommentRepeater.Controls.Count - 1
        Dim BindAllowed as Boolean= False
        If e.CommandName = "Sort" Then
            If CType(e.CommandArgument,SorterState)=SorterState.None
                If CType(ViewState("Grid_Modified_CommentSortDir"),SortDirections) = SortDirections._Asc And ViewState("Grid_Modified_CommentSortField").ToString()=CType(e.CommandSource,Controls.Sorter).Field
                    ViewState("Grid_Modified_CommentSortDir")=SortDirections._Desc
                Else
                    ViewState("Grid_Modified_CommentSortDir")=SortDirections._Asc
                End If
            Else
                ViewState("Grid_Modified_CommentSortDir")=CInt(CType(e.CommandSource,Controls.Sorter).State)
            End If
            Dim sf As Grid_Modified_CommentDataProvider.SortFields = 0
            ViewState("Grid_Modified_CommentSortField")=[Enum].Parse(sf.GetType(),CType(e.CommandSource,Controls.Sorter).Field)
            ViewState("Grid_Modified_CommentPageNumber") = 1
            BindAllowed = True
        End If

        If e.CommandName="Navigate" Then
            ViewState("Grid_Modified_CommentPageNumber") = Int32.Parse(e.CommandArgument.ToString())
            BindAllowed = True
        End If
        If e.CommandName="ChangePageSize" Then
            ViewState("Grid_Modified_CommentPageSize") = Int32.Parse(CType(e.CommandArgument,Integer())(0).ToString())
            ViewState("Grid_Modified_CommentPageNumber") = Int32.Parse(CType(e.CommandArgument,Integer())(1).ToString())
            BindAllowed = True
        End If
        If BindAllowed Then
            Grid_Modified_CommentBind
        End If
    End Sub
'End Grid Grid_Modified_Comment ItemCommand event
...
'OnInit Event Body @1-55399B2A
        ...
        Grid_Modified_CommentData = New Grid_Modified_CommentDataProvider()
        Grid_Modified_CommentOperations = New FormSupportedOperations(False, True, False, False, False)
