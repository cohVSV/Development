...
Partial Class FunnelBrowseApplications
   Inherits CCPage
   ...
   Friend Function GenerateStatusOutput(funnelLeadUUID As String, status As String, actionedBy As String, actionedDate As String, acceptedStudentID As String, enrolmentYear As String) As String
      Select Case status
         Case AppliedStudentStatusValue
            ...
            Dim processApplicationLink = $"<a href='{processApplicationUrl}' target='_blank'>Process application</a>"
         ...
         Case AcceptedStudentStatusValue
            If (acceptedStudentID.Length > 0) Then
               ...
               Return $"Accepted {actionedDate} by {actionedBy} - Student ID {studentSummaryLink}"
            Else
               Return $"Accepted {actionedDate} by {actionedBy} - No Student ID recorded"
            End If
         Case RejectedStudentStatusValue
            Return $"Rejected {actionedDate} by {actionedBy}"
         Case Else
            Return ""
      End Select
   End Function
      ...
      Return $"select
                ...
                where
                  ...
                  (
                     ...
                     or
                     (
                        (@status = '{AppliedStudentStatusValue}')
                        and (vfl.AcceptedStudentID is null)
                     )
                     or
                     (
                        (@status = '{AcceptedStudentStatusValue}')
                        and (vfl.AcceptedStudentID is not null)
                     )
                  )
                order by
