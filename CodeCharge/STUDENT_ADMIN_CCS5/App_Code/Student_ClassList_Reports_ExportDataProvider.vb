...
'Grid Students_Grid Item Class @25-0872507D
...
Implements IDataItem
    ...
    Public LearningAdvisorEmail As TextField
    Public LearningAdvisorEmailHref As Object
    Public LearningAdvisorEmailHrefParameters As LinkParameterCollection
    ...
    LearningAdvisorEmail = New TextField("",Nothing)
    LearningAdvisorEmailHrefParameters = New LinkParameterCollection()
    ...
    Public Default Property Field(ByVal fieldName As String) as FieldBase Implements _
        ...
        Get
            Select fieldName
                ...
                Case "LAD"
                    Return Me.LAD
                Case "LearningAdvisorEmail"
                    Return Me.LearningAdvisorEmail
                Case "CLASS_CODE"
        ...
        Set (ByVal Value As FieldBase)
            Select fieldName
                ...
                Case "LearningAdvisorEmail"
                    Me.LearningAdvisorEmail = CType(Value,TextField)
                Case "CLASS_CODE"
...
'Grid Students_Grid Data Provider Class Variables @25-C671BC7C
    ...
    Public Expr166  As TextParameter
'End Grid Students_Grid Data Provider Class Variables

'Grid Students_Grid Data Provider Class GetResultSet Method @25-BEFA5D57

    Public Sub New()
        Select_=new SqlCommand("SELECT *, iif(isnull(LAd, '') not in ('NO_SST', 'N-A', ''), concat(rtrim(lower(LAd)), '@vs" & _
          "v.vic.edu.au'), null) as LearningAdvisorEmail" & vbCrLf & _
          "FROM view_Class_Contact_List_04022011" & vbCrLf & _
          "WHERE ( ENROLMENT_YEAR =year(getdate()) )" & vbCrLf & _
          "AND SUBJECT_ID = {List_Subject_id}" & vbCrLf & _
          "AND SEMESTER LIKE '{List_SEMESTER}%'" & vbCrLf & _
          "AND SUBJ_ENROL_STATUS LIKE '{List_SUBJECT_ENROL_STATUS}%'" & vbCrLf & _
          "AND STAFF_ID = '{Expr0}' ",Settings.connDECVPRODSQL2005DataAccessObject)
        Count=new SqlCommand("SELECT COUNT(*) FROM (SELECT *, iif(isnull(LAd, '') not in ('NO_SST', 'N-A', ''), concat(r" & _
          "trim(lower(LAd)), '@vsv.vic.edu.au'), null) as LearningAdvisorEmail" & vbCrLf & _
          "FROM view_Class_Contact_List_04022011" & vbCrLf & _
          "WHERE ( ENROLMENT_YEAR =year(getdate()) )" & vbCrLf & _
          "AND SUBJECT_ID = {List_Subject_id}" & vbCrLf & _
          "AND SEMESTER LIKE '{List_SEMESTER}%'" & vbCrLf & _
          "AND SUBJ_ENROL_STATUS LIKE '{List_SUBJECT_ENROL_STATUS}%'" & vbCrLf & _
          "AND STAFF_ID = '{Expr0}' ) cnt",Settings.connDECVPRODSQL2005DataAccessObject)
    End Sub
...
'Before build Select @25-C8A5D3BD
        Dim E as Exception = Nothing
        Select_.Parameters.Clear()
        CType(Select_,SqlCommand).AddParameter("List_Subject_id",UrlList_Subject_id, "")
        CType(Select_,SqlCommand).AddParameter("List_SEMESTER",UrlList_SEMESTER, "")
        CType(Select_,SqlCommand).AddParameter("List_SUBJECT_ENROL_STATUS",UrlList_SUBJECT_ENROL_STATUS, "")
        CType(Select_,SqlCommand).AddParameter("Expr0",Expr166, "")
...
'After execute Select tail @25-E89E279E
            Dim i as Integer
            For i = 0 To dr.Count - 1
                ...
                item.LearningAdvisorEmail.SetValue(dr(i)("LearningAdvisorEmail"),"")
                item.LearningAdvisorEmailHref = dr(i)("LearningAdvisorEmail")
