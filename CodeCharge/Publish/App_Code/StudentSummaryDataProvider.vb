...
'Record viewStudentSummary_Detail Item Class @4-628A91C5
...
'Record viewStudentSummary_Detail Data Provider Class Constructor @4-90752CF0
    ...
    Public Sub New()
        Select_=new TableCommand("SELECT Region, Area, rtrim(surname) AS surname, rtrim(first_name) AS first_name, " & vbCrLf & _
          ...
          "erBy}", New String(){"student_id1122","And","ENROLMENT_YEAR1123"},Settings.connDECVPRODSQL2005DataAccessObject )
        Select_.OrderBy=""
    End Sub
...
'Record viewStudentSummary_Detail BeforeBuildSelect @4-0536C840
        Select_.Parameters.Clear()
        CType(Select_,TableCommand).AddParameter("student_id1122",Urlstudent_id, "","viewStudentSummary_Details.STUDENT_ID",Condition.Equal,False)
        CType(Select_,TableCommand).AddParameter("ENROLMENT_YEAR1123",UrlENROLMENT_YEAR, "","viewStudentSummary_Details.ENROLMENT_YEAR",Condition.Equal,False)
...
'Record viewStudentSummary_Detail AfterExecuteSelect @4-7F188549
...
'Grid Grid_Modified_Comment Item Class @428-429C297C
Public Class Grid_Modified_CommentItem 
Implements IDataItem
    Public errors As NameValueCollection = New NameValueCollection()
    Protected rm As System.Resources.ResourceManager = CType(System.Web.HttpContext.Current.Application("rm"),System.Resources.ResourceManager)
    Public COMMENT As TextField
    ...
    Public RawData As DataRow = Nothing
End Class
'End Grid Grid_Modified_Comment Item Class

'Grid Grid_Modified_Comment Data Provider Class Header @428-DEC93FC0
Public Class Grid_Modified_CommentDataProvider
Inherits GridDataProviderBase
'End Grid Grid_Modified_Comment Data Provider Class Header

'Grid Grid_Modified_Comment Data Provider Class Variables @428-F1FF3722
...
'End Grid Grid_Modified_Comment Data Provider Class Variables

'Grid Grid_Modified_Comment Data Provider Class GetResultSet Method @428-6FB9438A

    Public Sub New()
        Select_=new SqlCommand("SELECT TOP {SqlParam_endRecord} COMMENT_ID, COMMENT, LAST_ALTERED_BY, LAST_ALTERED_DATE, C" & _
          "OMMENT_TYPE" & vbCrLf & _
          "FROM STUDENT_COMMENT" & vbCrLf & _
          "WHERE COMMENT_TYPE in ('MODIFIED_SUBJECT', 'CUSTOM_LEARNING')" & vbCrLf & _
          "AND STUDENT_ID = {STUDENT_ID}" & vbCrLf & _
          "AND YEAR(LAST_ALTERED_DATE) = {ENROLMENT_YEAR}  {SQL_OrderBy}",Settings.connDECVPRODSQL2005DataAccessObject)
        Count=new SqlCommand("SELECT COUNT(*) FROM (SELECT COMMENT_ID, COMMENT, LAST_ALTERED_BY, LAST_ALTERED_DATE, COMM" & _
          "ENT_TYPE" & vbCrLf & _
          "FROM STUDENT_COMMENT" & vbCrLf & _
          "WHERE COMMENT_TYPE in ('MODIFIED_SUBJECT', 'CUSTOM_LEARNING')" & vbCrLf & _
          "AND STUDENT_ID = {STUDENT_ID}" & vbCrLf & _
          "AND YEAR(LAST_ALTERED_DATE) = {ENROLMENT_YEAR}) cnt",Settings.connDECVPRODSQL2005DataAccessObject)
    End Sub
'End Grid Grid_Modified_Comment Data Provider Class GetResultSet Method

'Grid Grid_Modified_Comment Data Provider Class GetResultSet Method @428-D8C604EA
    Public Function GetResultSet(ByRef _PagesCount As Integer, ops As FormSupportedOperations) As Grid_Modified_CommentItem()
'End Grid Grid_Modified_Comment Data Provider Class GetResultSet Method

'Before build Select @428-A84CD653
        Dim E as Exception = Nothing
        Select_.Parameters.Clear()
        CType(Select_,SqlCommand).AddParameter("STUDENT_ID",UrlSTUDENT_ID, "")
        CType(Select_,SqlCommand).AddParameter("ENROLMENT_YEAR",UrlENROLMENT_YEAR, "")
...
'Execute Select @428-E03E496C
        Dim ds As DataSet = Nothing
        _PagesCount=0
        Dim result(-1) As Grid_Modified_CommentItem
        If ops.AllowRead Then
            Try
                If RecordsPerPage > 0 Then
                    ds=ExecuteSelect((PageNumber-1)*RecordsPerPage,RecordsPerPage)
...
'After execute Select @428-89FE6AD5
                If Not IsNothing(E) Then Throw E
            End Try
            Dim dr As DataRowCollection = ds.Tables(tableIndex).Rows
            result = New Grid_Modified_CommentItem(dr.Count-1) {}
'End After execute Select

'After execute Select @428-79D6687E
            Dim j As Integer
'End After execute Select

'After execute Select tail @428-E6F360CD
            Dim i as Integer
            For i = 0 To dr.Count - 1
...
'Grid Data Provider tail @428-A61BA892
End Class
'End Grid Data Provider tail
